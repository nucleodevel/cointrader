...
public class MercadoBitcoinApiService extends ApiService {
	...
	private List<Order> getUserOrders(RecordFilter filter) throws ApiProviderException {
		...
		JsonObject jsonResponse = makePrivateRequest(getParams(filter), "list_orders", ApiVersion.V3);

		JsonArray jsonArray = jsonResponse != null ? jsonResponse.get("orders").getAsJsonArray() : new JsonArray();
	...
	private Order v1CreateOrder(Order order) throws ApiProviderException {
		...
		Order newOrder = new Order(order.getCoin(), order.getCurrency(), order.getSide(), order.getCoinAmount(),
				order.getCurrencyPrice());
	...
	private JsonHashMap getParams(Order order, ApiVersion version) throws ApiProviderException {
		...
		try {
			...
			if (version == ApiVersion.V3) {
				if (getCoin() != null && getCurrency() != null)
					params.put("coin_pair",
							getCurrency().getValue().toUpperCase() + getCoin().getValue().toUpperCase());
				...
				if (getCoin() != null && getCurrency() != null)
					params.put("pair",
							getCoin().getValue().toLowerCase() + "_" + getCurrency().getValue().toLowerCase());
				if (order.getSide() != null)
					params.put("type", order.getSide() == RecordSide.BUY ? "buy"
	...
	private JsonHashMap getParams(RecordFilter filter) throws ApiProviderException {
		...
		try {
			...
			if (filter.getCoin() != null && filter.getCurrency() != null)
				params.put("coin_pair",
						filter.getCurrency().getValue().toUpperCase() + filter.getCoin().getValue().toUpperCase());
			if (filter.getSide() != null)
				params.put("order_type",
						filter.getSide() == RecordSide.BUY ? "1" : (filter.getSide() == RecordSide.SELL ? "2" : null));
			if (filter.getStatus() != null)
				params.put("status_list",
	...
	private Balance getBalance(JsonObject balanceJsonObject) {
		...
		BigDecimal currencyLocked = getCurrency() == Currency.BRL ? brlTotal.subtract(brlAvailable) : null;
	...
	private Order getOrder(JsonObject jsonObject) {
		...
		OrderStatus status = statusInt == 2 ? OrderStatus.ACTIVE
				: (statusInt == 3 ? OrderStatus.CANCELED : (statusInt == 4 ? OrderStatus.COMPLETED : null));
		...
		List<Operation> operations = new ArrayList<Operation>();
		JsonArray jsonOperationArray = jsonObject.get("operations") == null ? new JsonArray()
