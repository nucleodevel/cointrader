		...
		for (;;) {
			...
			try {
				...
				if (report.getTicker() != null) {
					...
					System.out.println("  My participation: " + decFmt.format(
							report.getMy24hCoinVolume().doubleValue() / report.getTicker().getVol().doubleValue()));
					System.out
							.println("  Last 3 hour volume: " + decFmt.format(report.getTicker().getLast3HourVolume()));
				}
				...
				if (report.getTicker() != null) {
					...
					if (userConfiguration.getMaxInterval(RecordSide.BUY) != null)
						System.out.println("  Max accepted inactivity time for buying: "
					...
					if (userConfiguration.getMaxInterval(RecordSide.SELL) != null)
						System.out.println("  Max accepted inactivity time for selling: "
