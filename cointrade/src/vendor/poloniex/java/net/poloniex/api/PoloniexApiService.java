...
public class PoloniexApiService extends ApiService {
	...
	protected  void makeActionInConstructor() throws ApiProviderException {
		...
		System.out.println(getOperationList(null, null));
	}
	...
	public OrderBook getOrderBook() throws ApiProviderException {
		JsonObject orderBookJsonObject = (JsonObject) makePoloniexRequest("returnOrderBook"); 
	...
	public List<Operation> getOperationList(Calendar from, Calendar to) throws ApiProviderException {
		...
		JsonArray jsonArray = makePoloniexRequest("returnTradeHistory").getAsJsonArray();
		...
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			operation.getCreationDate().setTime(new Date(jsonObject.get("date").toString()));			
	...
	private JsonElement makePoloniexRequest(String method, String ... pathParams) throws ApiProviderException {
